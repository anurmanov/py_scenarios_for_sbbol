...
class Node1(Node):
	...
	def run_method(self, text_preprocessing_result: TextPreprocessingResult, user: User, params: Dict[str, Any] = None) -> List[Command]:
		import random
		from utils.jinja_filters_custom import ONBOARDING_CONFIGS
		...
		images = ONBOARDING_CONFIGS['grid_card']['background_images'][images_set_name]
		card = {
			"type": "grid_card",
			"items": [
				{
		...
		answer.set_pronounce_text(ONBOARDING_CONFIGS.get('pronounce_text', 'pronounce_text is not defined in on_boarding.yml'))
		answer.add_bubble(ONBOARDING_CONFIGS.get('bubble_text', 'bubble_text is not defined in on_boarding.yml'))
